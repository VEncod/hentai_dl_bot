...
# Default search source: 'both' | 'hcity' | 'oppai'
DEFAULT_SOURCE = "both"

BUTTON_HENTAICITY = "🏙️ HentaiCity (Default)"
BUTTON_HENTAI_TV = "🌐 Hentai.tv (Default)"  # Backwards compatibility alias
...
def get_main_reply_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        [
            [
                KeyboardButton(BUTTON_HENTAICITY),
...
async def get_user_search_source(user_id: int) -> str:
    """Get the active search source preference for a user ('both', 'hcity', 'oppai')."""
    try:
        db = get_db()
        doc = await db.user_preferences.find_one({"user_id": user_id})
        if doc and doc.get("search_source"):
            src = doc["search_source"]
            if src == "htv":
                return "hcity"
            return src
...
async def set_user_search_source(user_id: int, source: str) -> None:
    """Set the active search source preference for a user ('both', 'hcity', 'oppai')."""
    try:
        db = get_db()
